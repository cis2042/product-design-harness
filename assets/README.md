# Visual Plates

These editorial plates make Product Design Harness easier to read as a handbook.

| File | Use |
|---|---|
| `ux3-logo.jpeg` | Official UX3 wordmark used by the project site. |
| `product-design-harness-cover.svg` | Cover or presentation opener. |
| `ux3-model.svg` | Plate 01: UX3 operating model and Decision Kernel. |
| `intersections.svg` | Plate 02: UX3 intersection notes. |
| `harness-sequence.svg` | Plate 03: operating sequence. |
| `decision-gate.svg` | Plate 04: gate logic. |
| `product-vs-coding-harness.svg` | Plate 05: product and coding harness comparison. |

The visual plates are SVG and suitable for README, docs, slide decks, or book layouts. The UX3 wordmark is the original JPEG source supplied by the author.
