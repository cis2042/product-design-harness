...
同一套知識有兩個入口：人類閱讀專案網站與 handbook；agent 讀取
`SKILL.md`、`llms.txt`、prompts、schemas、rules 與 golden examples。
...
1. 把 `SKILL.md` 載入為 harness skill 或 system instruction。
2. 依序載入 `schemas/session-config.schema.json`、`knowledge/ontology.json`、`knowledge/rules.json`、`prompts/start-review.md`。
3. 每份輸出都用 `scripts/check_review.py` 驗證；JSON Schema 檢查結構與條件，validator 另外檢查 weakest-flow 與 headline-tier。
