...
server、SDK に依存しません。人はサイトと handbook、agent は `SKILL.md`、
`llms.txt`、prompts、schemas、rules、golden examples を入口として使い、
...
agent に接続するときは `SKILL.md` を読み込み、その後
`schemas/session-config.schema.json`、`knowledge/ontology.json`、
`knowledge/rules.json`、`prompts/start-review.md` の順に使います。
すべての出力は `schemas/review-result.schema.json` で検証します。
