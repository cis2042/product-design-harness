...
agent 安装 `.agents/skills/product-design-harness/` 完整 bundle，再使用
...
导入 agent 时，加载 `.agents/skills/product-design-harness/SKILL.md`，并保留
...
| Path | 用途 |
|---|---|
| `.agents/skills/product-design-harness/SKILL.md` | 可安装的 skill entrypoint，包含完整 runtime resources。 |
| `SKILL.md` | 供 repository 阅读者使用的 source contract；installer 会跳过它并选择完整 bundle。 |
