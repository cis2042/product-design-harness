...
agent 入口是 `SKILL.md`、`llms.txt`、prompts、schemas、rules 与 golden examples。
...
导入 agent 时，加载 `SKILL.md`，再依次使用
`schemas/session-config.schema.json`、`knowledge/ontology.json`、
`knowledge/rules.json`、`prompts/start-review.md`。所有输出都要用
`schemas/review-result.schema.json` 验证。
